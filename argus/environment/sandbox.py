...
def hydrate_sandbox(environment_state: dict) -> Sandbox:
    """Build a real Sandbox from a Scenario's environment_state dict, so
    an approved tool call can actually run against it instead of the
    demo/benchmark only printing what it would have done."""
    state = SandboxState()
    for c in environment_state.get("contacts", []):
        state.contacts.append(Contact(name=c["name"], email=c["email"]))
    for e in environment_state.get("emails", []):
        state.emails[e["id"]] = EmailRecord(
            id=e["id"], sender=e.get("sender", "unknown"),
            recipient=e.get("recipient", ""), subject=e.get("subject", ""),
            body=e.get("body", ""),
        )
    for f in environment_state.get("files", []):
        state.files[f["path"]] = FileRecord(
            path=f["path"], content=f.get("content", ""),
            is_temporary=f.get("is_temporary", False),
            important=f.get("important", False),
        )
    state.granted_permissions = set(environment_state.get("granted_permissions", []))
